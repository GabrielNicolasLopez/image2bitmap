...
function invertBitmap(bitmap) {
  return {
    width: bitmap.width,
    height: bitmap.height,
    pixels: bitmap.pixels.map((row) => row.map((pixel) => (pixel ? 0 : 1))),
  };
}
...
function handleImageUpload(file) {
  if (!file) return;
  imageInputName.textContent = file.name;
...
function generateFromCurrentImage(saveHistory = false) {
  ...
  const nextBitmap = imageDataToBitmap(imageData, width, height, {
    threshold: Number(thresholdInput.value),
  });
  state.bitmap = invertInput.checked ? invertBitmap(nextBitmap) : nextBitmap;
  updateAllOutputs();
}

function toggleInvert() {
  pushHistory();
  state.bitmap = invertBitmap(state.bitmap);
  updateAllOutputs();
}
...
function updateStats() {
  ...
  editorMeta.textContent = `${width} × ${height}`;
}
...
function resetAll() {
  ...
  imageInput.value = '';
  imageInputName.textContent = 'Ningun archivo seleccionado';
  widthInput.value = 64;
  ...
  zoomInput.value = 7;
  zoomValue.textContent = '7x';
  originalPreview.removeAttribute('src');
  originalPreview.style.display = 'none';
  originalEmptyState.style.display = 'inline-flex';
...
invertInput.addEventListener('change', toggleInvert);
